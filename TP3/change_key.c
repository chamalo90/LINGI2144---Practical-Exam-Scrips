...
uint8_t session_key[16]        = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

/*test challenge*/
uint8_t datatest[8]          = {0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88};
uint8_t dataset_2[16]        = {0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff};
/*APDU Declaration*/
...
uint8_t changeKey_apdu[31] = {0x90,0xC4,0x00,0x00,0x19,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
void
crc(uint8_t *pbtData, size_t szLen, uint8_t *pbtCrc);
...
int main(int argc, char *argv[])
{
    ...
    if(selectApplication(0x000004) == 0)
    {
...
unsigned short update_crc16(unsigned short crc, unsigned char c) { unsigned short i, v, tcrc = 0;

v = (crc ^ c) & 0xff; for (i = 0; i < 8; i++) { tcrc = ( (tcrc ^ v) & 1 ) ? ( tcrc >> 1 ) ^ 0x8408 : tcrc >> 1; v >>= 1; } return ((crc >> 8) ^ tcrc) & 0xffff; }
...
int changeKey()
{
  ...
  //  abtTx[6] = (AID >> 8 ) & 0xff; /*data 1*/
   // abtTx[5] = AID & 0xff;         /*data 2*/
    ...
    unsigned int old_key_id;
    printf("Enter the key ID you want to change : ");
    scanf("%1x",&old_key_id); // Retrieve the first 6 hexa digits introduced
    abtTx[5] = old_key_id;
    
    uint8_t final[24];
    uint8_t new_key_tab[16];
    uint8_t old_key_tab[16];
    uint8_t xored_key_tab[16];
    uint8_t new_key_tab_ciphered[24];
    uint8_t crc1[2];
    uint8_t crc2[2];
    //uint8_t new_key;
    printf("Enter the new key you want: ");
    int i;
    for (i=0; i<16 ; i++)
    {
        scanf("%02x",&new_key_tab[i]); // Retrieve the first 6 hexa digits introduced
    }

    for (i=0; i<16 ; i++)
    {
        scanf("%02x",&old_key_tab[i]); // Retrieve the first 6 hexa digits introduced
    }


    xor(old_key_tab, new_key_tab,xored_key_tab);
    crc_16(xored_key_tab, 16);
    crc_16(new_key_tab, 16);


    memcpy(&final[0], xored_key_tab, 16*sizeof(uint8_t));
    memcpy(&final[16], crc1, 2*sizeof(uint8_t));
    memcpy(&final[18], crc2, 2*sizeof(uint8_t));
    memcpy(&final[20], &abtTx[28], 4*sizeof(uint8_t));
    //final[0] = old_key_id;

    print_hex(final,24);
    encrypt(final, new_key_tab_ciphered, session_key, &session_key[8]);
    print_hex(new_key_tab_ciphered,24);

    memcpy(&abtTx[6],new_key_tab_ciphered,24*sizeof(uint8_t));
...
int changeMasterKey()
{
  ...
  //  abtTx[6] = (AID >> 8 ) & 0xff; /*data 1*/
   // abtTx[5] = AID & 0xff;         /*data 2*/
    ...
    unsigned int old_key_id = 0;
    /*printf("Enter the key ID you want to change : ");
    scanf("%1x",&old_key_id); // Retrieve the first 6 hexa digits introduced
    abtTx[5] = old_key_id;
    */
    uint8_t final[24];
    uint8_t new_key_tab[16] = {0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff};
...
//{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
   ...
   /* printf("Enter the new key you want: ");
    ...
    crc(new_key_tab, 16, crc1);

    memcpy(&final[0], new_key_tab, 16*sizeof(uint8_t));
    final[16] = crc1[1];
    final[17] = crc1[0];
    //memcpy(&final[16], crc1, 2*sizeof(uint8_t));
    //final[17] = final[17] ^ 0x01;
    final[18] = 0x00;
    final[19] = 0x00;
    final[20] = 0x00;
    final[21] = 0x00;
    ...
    print_hex(final,24);
    encrypt(final, new_key_tab_ciphered, &session_key[0], &session_key[8]);
    print_hex(new_key_tab_ciphered,24);
...
int authentification(uint8_t key_index,  uint8_t * key1, uint8_t * key2, uint8_t * challenge)
{
    ...
    memcpy(&session_key[4], output,4*sizeof(uint8_t));
    memcpy(&session_key[12], &output[4],4*sizeof(uint8_t));
    //memcpy(&session_key[0], output,4*sizeof(uint8_t));
    //memcpy(&session_key[8], &output[4],4*sizeof(uint8_t));
    //memcpy(&session_key[12], output,4*sizeof(uint8_t));
    //memcpy(&session_key[8], &output[4],4*sizeof(uint8_t));

    memcpy(&session_key[0], challenge,4*sizeof(uint8_t));
    memcpy(&session_key[8], &challenge[4],4*sizeof(uint8_t));
    //memcpy(&session_key[4], challenge,4*sizeof(uint8_t));
    //memcpy(&session_key[12], &challenge[4],4*sizeof(uint8_t));
    //memcpy(&session_key[4], challenge,4*sizeof(uint8_t));
    //memcpy(&session_key[0], &challenge[4],4*sizeof(uint8_t));
