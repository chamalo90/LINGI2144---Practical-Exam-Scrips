...
uint8_t deleteApplication_apdu[9] = {0x90,0xDA,0x00,0x00,0x03,0x00,0x0f,0x01,0x00};
uint8_t listApplication1_apdu[5] = {0x90,0x6A,0x00,0x00,0x00};
uint8_t listApplication2_apdu[5] = {0x90,0xAF,0x00,0x00,0x00};
...
if(authentification(0, defaultkey, defaultkey, datatest) == 0)
    ...
    while (strncmp(input,"exit",5) != 0){
	 ...
	 unsigned int AID = 0x000000;
	 printf("Enter the ID of the application you want to delete : ");
	 scanf("%06x",&AID); // Retrieve the first 6 hexa digits introduced
	 printf("AID = 0x%06x\n",AID);
	 if(deleteApplication(AID) != 0){
	     printf("Failure\n");
	     return EXIT_FAILURE;
	 }
	 printf("Success\n");
	...
	int count;
	int i=0;
	if(listApplication(list,&count) != 0){
	     printf("Failure\n");
	     return EXIT_FAILURE;
	 }
	printf("Number of APP = %d\n",count);
	for (i; i< count ; i++){
	  printf("Application [AID] == 0x%06x\n",list[i]);
	}
	 printf("Success\n");
      } //end else if
...
int createApplication(unsigned int AID)
{
    ...
    int settings;
    printf("Enter the security settings [1B] : ");
    scanf("%02x",&settings); // Retrieve the first 2 hexa digits introduced
    abtTx[8] = settings & 0xff; // TODO CHANGE !!!!

    unsigned int nKeys = 0;
    while (nKeys < 1 || nKeys > 14) {   
	printf("Enter (decimal) the number of key of the app (1 for masterkey / 14 max) : ");
        scanf("%d",&nKeys); // Retrieve the first 6 hexa digits introduced
    }
    abtTx[9] =  nKeys & 0xff;
...
int listApplication(unsigned int *outputList, int *outputCount)
{
    ...
    /*prepare the data*/
    abtTx_size = 5;
    memcpy(abtTx,listApplication1_apdu,abtTx_size);
    ...
    int tmp;
    int count = 0;
    for (count; count<length/3; count++){
      tmp =  abtRx[(count*3)+2];
      tmp =  (tmp<<8) + abtRx[(count*3)+1];
      tmp =  (tmp<<8) + abtRx[(count*3)];
      outputList[count]=tmp;
      //printf("%d\n",tmp);
    }
    // now count == number of app already given
    *outputCount = count;
    if (status == 0x9100){return 0;}

    memcpy(abtTx,listApplication2_apdu,abtTx_size);
    
    #ifdef DEBUG
    /*debug message*/
    printf("List Second Part of Applications\n");
    #endif
    ...
    for (i; i<length/3; i++){
      tmp =  abtRx[(i*3)+2];
      tmp =  (tmp<<8) + abtRx[(i*3)+1];
      tmp =  (tmp<<8) + abtRx[(i*3)];
      outputList[count+i]=tmp;
      //printf("%d\n",tmp);
    }
    // now count == number of app already given
    *outputCount = count+i;
